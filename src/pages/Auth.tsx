...
export default function Auth() {
  ...
  const handleStudentSignin = async (e: React.FormEvent) => {
    ...
    try {
      // First get the email from student_id
      const { data: profile, error: profileError } = await supabase
        .from('profiles')
        .select('email')
        .eq('student_id', studentSignin.studentId)
        .single();
      ...
      const { error } = await signIn(profile.email, studentSignin.password);
  ...
  const handleLibrarianSignin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (librarianSignin.librarianId === 'LIB001' && librarianSignin.password === 'admin123') {
        // Sign in with hardcoded librarian account
